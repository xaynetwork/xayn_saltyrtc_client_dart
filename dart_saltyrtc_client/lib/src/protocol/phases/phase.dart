...
class Common {
  ...
  Common(this.crypto, this.role, this.tasks) : server = Server(crypto);
}
...
abstract class Phase<Data> {
  /// Data common to all phases and role.
  @protected
  final Common common;

  /// Data that is specific for a role.
  @protected
  final Data data;

  Phase(this.common, this.data);
  ...
  void _validateNonceCs(Nonce nonce) {
    ...
    final cspTheirs = peer.csp.theirs;
    ...
    peer.csp.setTheirs(nonce.combinedSequence);
  }
...
abstract class InitiatorPhase extends Phase<InitiatorData> {
  InitiatorPhase(Common common, InitiatorData data) : super(common, data);
...
abstract class ResponderPhase extends Phase<ResponderData> {
  ResponderPhase(Common common, ResponderData data) : super(common, data);
